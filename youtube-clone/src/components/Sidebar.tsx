...
export const  Sidebar=()=> {
  ...
  const madeByYoutube = [
    ...
    {
      icon: <TbBrandYoutubeKids className="text-xl" />,
      name: "Youtube Kids",
    },
    {
  ...
  return (
    <div className="w-2/12 bg-[#080808] pr-5 overflow-auto pb-8 sidebar">
      <ul style={{paddingTop:'10px'}} className="flex flex-col border-b-2 border-gray-700">
      ...
      <ul className="flex flex-col border-b-2 border-gray-700">
        <h4 className={`pl-6 py-3`}>Made By Youtube</h4>
        {madeByYoutube.map(({ icon, name }) => {
          return (
            <li key={name} className={`pl-6 py-3 hover:bg-zinc-600 `}>
              <a href="#" className="flex items-center gap-5">
                {icon}
      ...
      <p className="px-4 pt-3 text-sm text-zinc-400">
        This clone is by Ishwer Sharma.
      </p>
    </div>
  );
}